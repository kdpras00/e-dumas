...
<div class="container mx-auto px-4 py-8">
    <!-- Breadcrumb -->
    <nav class="flex mb-8 max-w-4xl mx-auto" aria-label="Breadcrumb">
        <ol class="inline-flex items-center space-x-1 md:space-x-3">
            ...
            <li>
                <div class="flex items-center">
                    ...
                    <span class="ml-1 text-sm font-medium text-white md:ml-2">Master RW</span>
                </div>
            </li>
        </ol>
    </nav>
    ...
    <!-- Header Card -->
    <div class="max-w-4xl mx-auto bg-white rounded-3xl shadow-2xl mb-8 relative overflow-hidden border border-white/20">
        <div class="p-10 flex flex-col md:flex-row justify-between items-center gap-4">
            <div>
                <h1 class="text-3xl font-bold text-blue-900 tracking-wide">Master RW</h1>
                <p class="text-gray-500 mt-1 text-sm">Kelola data Rukun Warga (RW)</p>
            </div>
            <a href="{{ route('admin.rw.create') }}" class="bg-gradient-to-r from-blue-600 to-blue-800 text-white font-bold py-3 px-8 rounded-full shadow-lg hover:shadow-blue-500/30 transition transform hover:-translate-y-0.5 flex items-center">
    ...
    <!-- Table Card -->
    <div class="max-w-4xl mx-auto bg-white rounded-3xl shadow-2xl border border-white/20 overflow-hidden">
        <div class="p-8">
                        ...
                        <tr class="hover:bg-blue-50/50 transition duration-150">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-center text-sm font-medium">
                                <div class="flex justify-center space-x-3">
                                    <a href="{{ route('admin.rw.edit', $rw->id) }}" class="text-blue-600 hover:text-blue-800 transition p-1 rounded-full hover:bg-blue-100">
                                        <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" />
                                        </svg>
                                    </a>
                                    <button onclick="openDeleteModal('{{ $rw->id }}', '{{ $rw->rw }}')" class="text-red-600 hover:text-red-800 transition p-1 rounded-full hover:bg-red-100">
                                        <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
...
<div id="deleteModal" class="fixed inset-0 z-50 hidden flex items-center justify-center p-4">
    ...
    <div class="bg-white rounded-2xl shadow-2xl max-w-sm w-full transform scale-100 transition-all relative z-10 overflow-hidden">
        <div class="p-6 text-center">
            ...
            <h3 class="text-xl font-bold text-gray-900 mb-2">Hapus RW?</h3>
            <p class="text-gray-500 mb-6">Apakah Anda yakin ingin menghapus RW <span id="deleteRwName" class="font-bold text-gray-800"></span>? Tindakan ini tidak dapat dibatalkan.</p>
...
<script>
    function openDeleteModal(id, name) {
        ...
        const nameEl = document.getElementById('deleteRwName');
        const form = document.getElementById('deleteForm');
        
        nameEl.textContent = name;
        form.action = `/admin/rw/${id}`;
